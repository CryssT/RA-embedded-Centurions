...
int main(void) {
  ...
  *(PCC_PORT_C) |= PCC_ENABLE_PORT_MASK;
  ...
  (((GPIO_map_tp) PORT_C)->PDDR) &= ~(BTN_1);

  (((GPIO_map_tp) PORT_C)->PDDR) &= ~(BTN_2);

  //3)Port function ---> PCR.Mux :001b=GPIO
  PCR_PORT_C[PTC12] |= PCR_GPIO_MODE_MASK;

  PCR_PORT_C[PTC13] |= PCR_GPIO_MODE_MASK;
  ...
  for(;;) {
	  ...
	  if(((((GPIO_map_tp) PORT_C)->PDIR) & (BTN_1)) && ((((GPIO_map_tp) PORT_C)->PDIR) & (BTN_2)))
	  {
	  ...
	  else if((((GPIO_map_tp) PORT_C)->PDIR) & (BTN_2)){
